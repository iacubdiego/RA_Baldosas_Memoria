...
export async function GET(
  ...
  try {
    ...
    if (id.match(/^[0-9a-fA-F]{24}$/)) {
      try { baldosa = await Baldosa.findById(id); } catch {}
    }
    if (!baldosa) {
    ...
    if (!baldosa || !baldosa.activo) {
      return NextResponse.json({ error: 'Baldosa no encontrada' }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: 'Error al obtener baldosa' }, { status: 500 });
  }
}
